...
| Stale release evidence | A live release uses an old audit from another commit or workspace. | Live release validation checks the passed audit, local-only preview status, workspace binding, tag, and HEAD commit before mutation. `Release Publish` also requires a successful `Release Rehearsal` run ID, an existing signed annotated tag for the publish commit, and regenerated release preview, checksums, verified attestations, and an attested release evidence bundle in the publish job. | Factory packet policy decisions, release preview audit fields, `release-rehearsal-evidence`, `release-publish-evidence`, signed tag verification logs, and `release-evidence-bundle.attestation.json`. | Promotion criteria should define the minimum evidence set for production-stable releases. |
...
steps are production-stable promotion criteria, signer eligibility rules, and
