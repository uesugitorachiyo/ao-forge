...
| Credential exposure | Release preview accidentally depends on write credentials or stores tokens in output. | The release preview workflow uses `contents: read`; preview commands do not require network access and do not store credentials. CI runs `scripts/check-public-repo-policy.sh` against tracked files to catch private-key markers, credential-shaped assignments, high-confidence tokens, and machine-local home paths before merge; the local release-readiness gate also keeps `gitleaks` as an optional deeper scan. | Workflow YAML, audit fields `mutates_releases=false` and `network_required=false`, `public-repo-policy-check.txt`, and local scan output. | Keep deeper third-party secret scanning in the local release-readiness checklist for defense in depth. |
...
- a CI public-repo policy scanner plus deeper local secret scanning in the
  recommended release-readiness gate.
