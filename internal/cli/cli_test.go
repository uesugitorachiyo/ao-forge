...
func readJSONFixture(t *testing.T, path string, target any) {
	...
	if err := json.Unmarshal(bytes, target); err != nil {
		t.Fatalf("parse JSON fixture %s: %v", path, err)
	}
}
...
func capturedOutputFamilyPassed(outputs []struct {
	Label    string `json:"label"`
	Families []struct {
...
}, label, family string) bool {
	for _, output := range outputs {
		if output.Label != label {
			continue
		}
		for _, item := range output.Families {
			if item.Family == family && item.Passed {
				return true
			}
		}
	}
...
func TestHelpExplainsFactoryTermsWithoutMarketingCopy(t *testing.T) {
	...
	for _, want := range []string{
		...
		"GoalRun evidence path linting",
		"GoalRun retained evidence retention audits",
		"GoalRun readiness audits",
	} {
...
func TestGoalRunCLIValidatesVerificationEvidence(t *testing.T) {
	root := repoRoot(t)
	validPath := filepath.Join(root, "examples", "goals", "ao2-weekend-hardening.goal-run-verification.json")
	code, stdout, stderr := runCLI("goal", "verification", "validate", "--verification", validPath, "--json")
	if code != 0 {
		t.Fatalf("goal verification validate exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	var summary struct {
		Status string `json:"status"`
		GoalID string `json:"goal_id"`
	...
	if err := json.Unmarshal([]byte(stdout), &summary); err != nil {
		t.Fatalf("parse verification summary: %v\n%s", err, stdout)
	}
	if summary.Status != "passed" || summary.GoalID != "ao2-weekend-hardening" {
		t.Fatalf("unexpected verification summary: %#v", summary)
	}

	invalidPath := filepath.Join(root, "examples", "goals", "invalid", "missing-security.goal-run-verification.invalid.json")
	code, stdout, stderr = runCLI("goal", "verification", "validate", "--verification", invalidPath, "--json")
	if code != 1 {
		t.Fatalf("invalid verification exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	if !strings.Contains(stdout, "missing required verification phase security_scan") {
		t.Fatalf("invalid verification stdout missing security failure:\n%s\nstderr:\n%s", stdout, stderr)
	}
}
...
func TestGoalRunCLIVerifiesEvidenceHashes(t *testing.T) {
	...
	if code != 0 {
		t.Fatalf("goal evidence verify exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	for _, want := range []string{
		"goal_evidence_verify=passed",
		"goal_run=examples/goals/ao2-pulse-handoff.goal-run.json",
		"goal_id=ao2-weekend-hardening",
		"evidence_verified=1",
	...
	if stderr != "" {
		t.Fatalf("goal evidence verify wrote stderr: %s", stderr)
	}

	code, stdout, stderr = runCLI("goal", "evidence", "verify", "--goal-run", handoffPath, "--json")
	if code != 0 {
		t.Fatalf("goal evidence verify --json exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	var summary struct {
	...
	if stderr != "" {
		t.Fatalf("goal evidence verify --json wrote stderr: %s", stderr)
	}

	code, stdout, stderr = runCLI("goal", "evidence", "verify")
	if code != 2 {
		t.Fatalf("goal evidence verify missing flag exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	if !strings.Contains(stderr, "missing required --goal-run") {
	...
	if !strings.Contains(stdout, "goal_evidence_verify=failed") ||
		!strings.Contains(stdout, "sha256 mismatch") {
		t.Fatalf("goal evidence verify stale stdout drifted:\n%s", stdout)
	}
...
func TestGoalRunCLILintsEvidencePaths(t *testing.T) {
	root := repoRoot(t)
	...
	if code != 0 {
		t.Fatalf("goal evidence lint exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	for _, want := range []string{
		"goal_evidence_lint=passed",
		...
		"evidence_path=docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-command-rsi-manifest-retention-proof.json status=passed",
		...
		"evidence_path=docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-foundry-rsi-next-improvement-task-retention-proof.json status=passed",
		"evidence_path=docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/bounded-rsi-improvement-chain-retention-proof.json status=passed",
		"evidence_path=docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-stack-rsi-chain-binding-readback-retention-proof.json status=passed",
	} {
		if !strings.Contains(stdout, want) {
			t.Fatalf("goal evidence lint stdout missing %q\n%s", want, stdout)
		}
	}
	if stderr != "" {
		t.Fatalf("goal evidence lint wrote stderr: %s", stderr)
	}

	code, stdout, stderr = runCLI("goal", "evidence", "lint", "--update-audit", auditPath)
	if code != 0 {
		t.Fatalf("goal evidence lint update audit exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	for _, want := range []string{
		"goal_evidence_lint=passed",
		"document=examples/goals/ao2-pulse-handoff.goal-run-update-audit.json",
		"document_type=goal_run_update_audit",
		"goal_id=ao2-weekend-hardening",
		"evidence_linted=1",
		"evidence_path=examples/goals/ao2-weekend-hardening.goal-run.json status=passed",
	} {
		if !strings.Contains(stdout, want) {
			t.Fatalf("goal evidence lint update audit stdout missing %q\n%s", want, stdout)
		}
	}
	if stderr != "" {
		t.Fatalf("goal evidence lint update audit wrote stderr: %s", stderr)
	}

	code, stdout, stderr = runCLI("goal", "evidence", "lint", "--goal-run", retainedPath, "--json")
	if code != 0 {
		t.Fatalf("goal evidence lint --json exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	var summary struct {
	...
	if err := json.Unmarshal([]byte(stdout), &summary); err != nil {
		t.Fatalf("goal evidence lint --json emitted invalid JSON: %v\n%s", err, stdout)
	}
	var lintDocument any
	if err := json.Unmarshal([]byte(stdout), &lintDocument); err != nil {
		t.Fatalf("goal evidence lint --json emitted invalid JSON document: %v\n%s", err, stdout)
	}
	if err := validateJSONSchemaValue(lintSchemaPath, lintDocument); err != nil {
		t.Fatalf("goal evidence lint --json failed schema validation: %v\n%s", err, stdout)
	}
	if summary.LintSchemaVersion != "ao.forge.goal-run-evidence-lint.v0.1" ||
		summary.DocumentType != "goal_run" ||
		summary.Status != "passed" ||
		summary.EvidenceLinted != 8 ||
		len(summary.Evidence) != 8 ||
		summary.Evidence[0].Status != "passed" ||
		summary.Evidence[1].Status != "passed" ||
		summary.Evidence[2].Status != "passed" ||
		summary.Evidence[3].Status != "passed" ||
		summary.Evidence[4].Status != "passed" ||
		summary.Evidence[5].Status != "passed" ||
		summary.Evidence[6].Status != "passed" ||
		summary.Evidence[7].Status != "passed" {
		t.Fatalf("goal evidence lint --json summary drifted: %+v", summary)
	}
	if stderr != "" {
		t.Fatalf("goal evidence lint --json wrote stderr: %s", stderr)
	}

	code, stdout, stderr = runCLI("goal", "evidence", "lint")
	if code != 2 {
		t.Fatalf("goal evidence lint missing flag exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	if !strings.Contains(stderr, "missing required --goal-run or --update-audit") {
		t.Fatalf("goal evidence lint missing flag stderr drifted: %s", stderr)
	}

	code, stdout, stderr = runCLI("goal", "evidence", "lint", "--goal-run", retainedPath, "--update-audit", auditPath)
	if code != 2 {
		t.Fatalf("goal evidence lint dual target exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	if !strings.Contains(stderr, "use exactly one of --goal-run or --update-audit") {
...
func TestRetainedRSIEvidenceIncludesFoundryImprovementGate(t *testing.T) {
	root := repoRoot(t)
	goalPath := filepath.Join(root, "examples", "goals", "ao2-retained-evidence.goal-run.json")
	commandProofPath := filepath.Join(root, "docs", "evidence", "goals", "ao2-weekend-hardening", "20260619T180000Z-verification", "ao-command-rsi-health-retention-proof.json")
	commandManifestProofPath := filepath.Join(root, "docs", "evidence", "goals", "ao2-weekend-hardening", "20260619T180000Z-verification", "ao-command-rsi-manifest-retention-proof.json")
	foundryProofPath := filepath.Join(root, "docs", "evidence", "goals", "ao2-weekend-hardening", "20260619T180000Z-verification", "ao-foundry-rsi-improvement-gate-retention-proof.json")
	foundryNextTaskProofPath := filepath.Join(root, "docs", "evidence", "goals", "ao2-weekend-hardening", "20260619T180000Z-verification", "ao-foundry-rsi-next-improvement-task-retention-proof.json")
	boundedChainProofPath := filepath.Join(root, "docs", "evidence", "goals", "ao2-weekend-hardening", "20260619T180000Z-verification", "bounded-rsi-improvement-chain-retention-proof.json")
	stackChainBindingProofPath := filepath.Join(root, "docs", "evidence", "goals", "ao2-weekend-hardening", "20260619T180000Z-verification", "ao-stack-rsi-chain-binding-readback-retention-proof.json")
	...
	readJSONFixture(t, goalPath, &goal)
	if !goalEvidencePathPresent(goal.LastIteration.Evidence, "docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-foundry-rsi-improvement-gate-retention-proof.json") {
		t.Fatalf("GoalRun does not retain AO Foundry RSI improvement gate evidence: %+v", goal.LastIteration.Evidence)
	}
	if !goalEvidencePathPresent(goal.LastIteration.Evidence, "docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-foundry-rsi-next-improvement-task-retention-proof.json") {
		t.Fatalf("GoalRun does not retain AO Foundry RSI next improvement task evidence: %+v", goal.LastIteration.Evidence)
	}
	if !goalEvidencePathPresent(goal.LastIteration.Evidence, "docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-command-rsi-manifest-retention-proof.json") {
		t.Fatalf("GoalRun does not retain AO Command RSI manifest validation evidence: %+v", goal.LastIteration.Evidence)
	}
	if !goalEvidencePathPresent(goal.LastIteration.Evidence, "docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/bounded-rsi-improvement-chain-retention-proof.json") {
		t.Fatalf("GoalRun does not retain bounded RSI improvement chain evidence: %+v", goal.LastIteration.Evidence)
	}
	if !goalEvidencePathPresent(goal.LastIteration.Evidence, "docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-stack-rsi-chain-binding-readback-retention-proof.json") {
		t.Fatalf("GoalRun does not retain AO stack RSI chain-binding readback evidence: %+v", goal.LastIteration.Evidence)
	}

	var commandProof struct {
		...
		} `json:"captured_outputs"`
	}
	readJSONFixture(t, commandProofPath, &commandProof)
	commandRSIFoundryPassed := false
	for _, output := range commandProof.CapturedOutputs {
		if output.Label != "ao-command-rsi-health" {
			continue
		}
		for _, family := range output.Families {
			if family.Family == "ao-foundry" && family.Passed {
				commandRSIFoundryPassed = true
			}
		}
	}
	if !commandRSIFoundryPassed {
		t.Fatalf("AO Command RSI retained proof missing passed ao-foundry family: %+v", commandProof.CapturedOutputs)
	}
	if len(commandProof.CapturedOutputs) != 1 ||
		!capturedOutputHasClaimLevel(commandProof.CapturedOutputs[0].ClaimLevels, "bounded_governed_rsi", "allowed", "passed") ||
		!capturedOutputHasClaimLevel(commandProof.CapturedOutputs[0].ClaimLevels, "full_autonomous_self_mutating_rsi", "denied", "blocked") {
		t.Fatalf("AO Command RSI retained proof missing claim levels: %+v", commandProof.CapturedOutputs)
	}

	var commandManifestProof struct {
		CapturedOutputs []struct {
			Label           string   `json:"label"`
			...
			EvidenceMarkers []string `json:"evidence_markers"`
			ClaimLevels     []struct {
				Claim    string `json:"claim"`
				Decision string `json:"decision"`
				Status   string `json:"status"`
	...
	readJSONFixture(t, commandManifestProofPath, &commandManifestProof)
	if len(commandManifestProof.CapturedOutputs) != 1 {
		t.Fatalf("AO Command RSI manifest retained proof must have one captured output: %+v", commandManifestProof.CapturedOutputs)
	}
	manifestOutput := commandManifestProof.CapturedOutputs[0]
	if manifestOutput.Label != "ao-command-rsi-manifest" ||
		manifestOutput.Command != "ao-command rsi manifest --manifest ../ao-architecture/overview/rsi-claim-evidence-manifest.json" ||
		manifestOutput.SchemaVersion != "ao.command.rsi-manifest.v0.1" ||
		manifestOutput.Status != "passed" ||
		manifestOutput.OperatorMode != "read_only" ||
		manifestOutput.MutatesRepositories ||
		!stringSliceContains(manifestOutput.EvidenceMarkers, "rollback_rehearsal.status=passed") ||
		!stringSliceContains(manifestOutput.EvidenceMarkers, "AO Command PR #31") ||
		!capturedOutputHasClaimLevel(manifestOutput.ClaimLevels, "bounded_governed_rsi", "allowed", "passed") ||
		!capturedOutputHasClaimLevel(manifestOutput.ClaimLevels, "full_autonomous_self_mutating_rsi", "denied", "blocked") {
		t.Fatalf("AO Command RSI manifest retained proof missing rollback-aware manifest validation: %+v", manifestOutput)
	}

	var foundryProof struct {
		SchemaVersion   string `json:"schema_version"`
		GoalID          string `json:"goal_id"`
		...
		CapturedOutputs []struct {
			...
			ActualImprovement   float64 `json:"actual_improvement_percent"`
			AutonomousClaim     string  `json:"autonomous_claim"`
			MutatesRepositories bool    `json:"mutates_repositories"`
		} `json:"captured_outputs"`
	}
	readJSONFixture(t, foundryProofPath, &foundryProof)
	if foundryProof.SchemaVersion != "ao.forge.goal-run-retained-evidence.v0.1" ||
		foundryProof.GoalID != "ao2-weekend-hardening" ||
		foundryProof.Iteration != "20260619T180000Z-verification" ||
		foundryProof.Phase != "verification" ||
		len(foundryProof.CapturedOutputs) != 1 {
		t.Fatalf("unexpected Foundry RSI retained proof envelope: %+v", foundryProof)
	}
	output := foundryProof.CapturedOutputs[0]
	if output.Label != "ao-foundry-rsi-improvement-gate" ||
		output.Command != "foundry pulse run" ||
		output.SchemaVersion != "ao.foundry.rsi-improvement-gate.v0.1" ||
		output.Status != "passed" ||
		output.BaselineScore != 90 ||
		output.CandidateScore != 100 ||
		output.RequiredImprovement != 5 ||
		output.ActualImprovement < output.RequiredImprovement ||
		output.AutonomousClaim != "measured_local_improvement" ||
		output.MutatesRepositories {
		t.Fatalf("unexpected Foundry RSI retained proof output: %+v", output)
	}

	var foundryNextTaskProof struct {
		SchemaVersion   string `json:"schema_version"`
		GoalID          string `json:"goal_id"`
		Iteration       string `json:"iteration"`
		Phase           string `json:"phase"`
	...
	readJSONFixture(t, foundryNextTaskProofPath, &foundryNextTaskProof)
	if foundryNextTaskProof.SchemaVersion != "ao.forge.goal-run-retained-evidence.v0.1" ||
		foundryNextTaskProof.GoalID != "ao2-weekend-hardening" ||
		foundryNextTaskProof.Iteration != "20260619T180000Z-verification" ||
		foundryNextTaskProof.Phase != "verification" ||
		len(foundryNextTaskProof.CapturedOutputs) != 1 {
		t.Fatalf("unexpected Foundry RSI next task retained proof envelope: %+v", foundryNextTaskProof)
	}
	nextTaskOutput := foundryNextTaskProof.CapturedOutputs[0]
	if nextTaskOutput.Label != "ao-foundry-rsi-next-improvement-task" ||
		nextTaskOutput.Command != "foundry pulse run" ||
		nextTaskOutput.SchemaVersion != "ao.foundry.rsi-next-improvement-task.v0.1" ||
		nextTaskOutput.Status != "ready" ||
		nextTaskOutput.RequiredImprovement != 5 ||
		nextTaskOutput.ActualImprovement < nextTaskOutput.RequiredImprovement ||
		nextTaskOutput.AutonomousClaim != "derived_local_next_improvement" ||
		nextTaskOutput.MutatesRepositories {
		t.Fatalf("unexpected Foundry RSI next task retained proof output: %+v", nextTaskOutput)
	}

	var boundedChainProof struct {
	...
	readJSONFixture(t, boundedChainProofPath, &boundedChainProof)
	if !strings.Contains(boundedChainProof.Summary, "bounded, governed RSI evidence chain") || !strings.Contains(boundedChainProof.Summary, "not a claim of full autonomous self-mutating RSI") {
		t.Fatalf("bounded RSI proof summary does not preserve claim boundary: %q", boundedChainProof.Summary)
	}
	labels := map[string]bool{}
	for _, output := range boundedChainProof.CapturedOutputs {
		labels[output.Label] = true
		if output.MutatesRepositories {
			t.Fatalf("bounded RSI proof captured mutating output: %+v", output)
		}
		if output.Label == "ao-foundry-rsi-improvement-gate" && output.ActualImprovement < output.RequiredImprovement {
			t.Fatalf("bounded RSI proof improvement gate does not meet threshold: %+v", output)
		}
		if output.Label == "ao-command-rsi-health" &&
			(!capturedOutputHasClaimLevel(output.ClaimLevels, "bounded_governed_rsi", "allowed", "passed") ||
				!capturedOutputHasClaimLevel(output.ClaimLevels, "full_autonomous_self_mutating_rsi", "denied", "blocked")) {
			t.Fatalf("bounded RSI proof AO Command output missing claim levels: %+v", output)
		}
	}
	for _, label := range []string{"ao-foundry-rsi-candidate", "ao-foundry-rsi-improvement-gate", "ao-foundry-rsi-next-improvement-task", "ao-command-rsi-health", "ao-command-rsi-manifest"} {
		if !labels[label] {
			t.Fatalf("bounded RSI proof missing captured output %q: %+v", label, boundedChainProof.CapturedOutputs)
		}
	}

	var stackChainBindingProof struct {
		SchemaVersion   string `json:"schema_version"`
		GoalID          string `json:"goal_id"`
		Iteration       string `json:"iteration"`
		Phase           string `json:"phase"`
	...
	readJSONFixture(t, stackChainBindingProofPath, &stackChainBindingProof)
	if stackChainBindingProof.SchemaVersion != "ao.forge.goal-run-retained-evidence.v0.1" ||
		stackChainBindingProof.GoalID != "ao2-weekend-hardening" ||
		stackChainBindingProof.Iteration != "20260619T180000Z-verification" ||
		stackChainBindingProof.Phase != "verification" ||
		len(stackChainBindingProof.CapturedOutputs) != 1 {
		t.Fatalf("unexpected AO stack RSI chain-binding retained proof envelope: %+v", stackChainBindingProof)
	}
	stackChainBindingOutput := stackChainBindingProof.CapturedOutputs[0]
	if stackChainBindingOutput.Label != "ao-stack-rsi-chain-binding-readback" ||
		stackChainBindingOutput.Command != "ao2-control-plane scripts/verify_ao_stack_rsi_chain_binding_readback.py" ||
		stackChainBindingOutput.SchemaVersion != "ao2.cp-ao-stack-rsi-chain-binding-readback.v1" ||
		stackChainBindingOutput.Status != "passed" ||
		stackChainBindingOutput.OperatorMode != "read_only" ||
		stackChainBindingOutput.MutatesRepositories ||
		!stringSliceContains(stackChainBindingOutput.EvidenceMarkers, "chain=blueprint->foundry->forge->covenant->ao2->control-plane") ||
		!stringSliceContains(stackChainBindingOutput.EvidenceMarkers, "control_plane_ao_stack_rsi_chain_binding_readback=passed") ||
		!stringSliceContains(stackChainBindingOutput.EvidenceMarkers, "control_plane_approves_rsi_claims=false") ||
		!capturedOutputHasClaimLevel(stackChainBindingOutput.ClaimLevels, "bounded_governed_rsi", "allowed", "passed") ||
		!capturedOutputHasClaimLevel(stackChainBindingOutput.ClaimLevels, "full_autonomous_self_mutating_rsi", "denied", "blocked") {
		t.Fatalf("AO stack RSI chain-binding retained proof drifted: %+v", stackChainBindingOutput)
	}
}
...
func capturedOutputHasClaimLevel(claimLevels []struct {
	Claim    string `json:"claim"`
	Decision string `json:"decision"`
	Status   string `json:"status"`
}, claim, decision, status string) bool {
	for _, level := range claimLevels {
		if level.Claim == claim && level.Decision == decision && level.Status == status {
			return true
		}
	}
	return false
...
func TestGoalRunCLIAuditsRetainedEvidenceRetention(t *testing.T) {
	root := repoRoot(t)
	...
	code, stdout, stderr := runCLI("goal", "evidence", "retention", "--artifact", artifactPath, "--now", now)
	if code != 0 {
		t.Fatalf("goal evidence retention exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	for _, want := range []string{
		"goal_evidence_retention=passed",
		"artifact=docs/evidence/goals/ao2-weekend-hardening/20260619T143000Z-implementation/ao2-pulse-handoff-retention-proof.json",
		"goal_id=ao2-weekend-hardening",
		"phase=implementation",
		"retention_class=loop_evidence",
		"retained_at=2026-06-19T14:30:00Z",
		"retention_status=active_retention",
		"cleanup_review_status=not_eligible_active_goal",
		"age_days=0",
	} {
		if !strings.Contains(stdout, want) {
			t.Fatalf("goal evidence retention stdout missing %q\n%s", want, stdout)
		}
	}
	if stderr != "" {
		t.Fatalf("goal evidence retention wrote stderr: %s", stderr)
	}

	code, stdout, stderr = runCLI("goal", "evidence", "retention", "--artifact", artifactPath, "--now", now, "--json")
	if code != 0 {
		t.Fatalf("goal evidence retention --json exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	var summary struct {
	...
	if err := json.Unmarshal([]byte(stdout), &summary); err != nil {
		t.Fatalf("goal evidence retention --json emitted invalid JSON: %v\n%s", err, stdout)
	}
	var auditDocument any
	if err := json.Unmarshal([]byte(stdout), &auditDocument); err != nil {
		t.Fatalf("goal evidence retention --json emitted invalid JSON document: %v\n%s", err, stdout)
	}
	if err := validateJSONSchemaValue(auditSchemaPath, auditDocument); err != nil {
		t.Fatalf("goal evidence retention --json failed schema validation: %v\n%s", err, stdout)
	}
	if summary.RetentionAuditSchemaVersion != "ao.forge.goal-run-retained-evidence-audit.v0.1" ||
		summary.Status != "passed" ||
		summary.RetentionStatus != "active_retention" ||
		summary.CleanupReviewStatus != "not_eligible_active_goal" ||
		summary.AgeDays != 0 {
		t.Fatalf("goal evidence retention JSON drifted: %+v", summary)
	}
	if stderr != "" {
		t.Fatalf("goal evidence retention --json wrote stderr: %s", stderr)
	}

	terminalPath := filepath.Join(t.TempDir(), "terminal-retained-evidence.json")
	terminalArtifact := strings.ReplaceAll(`{
  "schema_version": "ao.forge.goal-run-retained-evidence.v0.1",
  "goal_id": "ao2-weekend-hardening",
  "iteration": "20260101T000000Z-complete",
  "phase": "complete",
  "summary": "Terminal retained evidence fixture for cleanup eligibility auditing.",
  "retention_policy": {
  ...
  "retention_metadata": {
	...
	if err := os.WriteFile(terminalPath, []byte(terminalArtifact), 0o644); err != nil {
		t.Fatalf("write terminal retained evidence fixture: %v", err)
	}
	code, stdout, stderr = runCLI("goal", "evidence", "retention", "--artifact", terminalPath, "--now", now)
	if code != 0 {
		t.Fatalf("goal evidence retention terminal exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	for _, want := range []string{
		"phase=complete",
		"retention_status=cleanup_review_eligible",
		"cleanup_review_status=eligible_after_review",
		"mandatory_retention_until=2026-04-01T00:00:00Z",
	} {
		if !strings.Contains(stdout, want) {
			t.Fatalf("goal evidence retention terminal stdout missing %q\n%s", want, stdout)
		}
	}
	if stderr != "" {
		t.Fatalf("goal evidence retention terminal wrote stderr: %s", stderr)
	}

	code, stdout, stderr = runCLI("goal", "evidence", "retention", "--artifact", releaseProvenancePath, "--now", now)
	if code != 0 {
		t.Fatalf("goal evidence retention release provenance exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	for _, want := range []string{
		"phase=complete",
		"retention_class=release_provenance",
		"retention_status=mandatory_retention",
		"cleanup_review_status=not_eligible_public_provenance",
	} {
		if !strings.Contains(stdout, want) {
			t.Fatalf("goal evidence retention release provenance stdout missing %q\n%s", want, stdout)
		}
	}
	if strings.Contains(stdout, "mandatory_retention_until=") {
		t.Fatalf("goal evidence retention release provenance should not emit a terminal cleanup date:\n%s", stdout)
	}
	if stderr != "" {
	...
	if err := json.Unmarshal([]byte(stdout), &auditDocument); err != nil {
		t.Fatalf("goal evidence retention promotion provenance --json emitted invalid JSON: %v\n%s", err, stdout)
	}
	if err := validateJSONSchemaValue(auditSchemaPath, auditDocument); err != nil {
		t.Fatalf("goal evidence retention promotion provenance --json failed schema validation: %v\n%s", err, stdout)
	}
	if !strings.Contains(stdout, `"retention_class": "promotion_provenance"`) ||
		!strings.Contains(stdout, `"retention_status": "mandatory_retention"`) ||
		!strings.Contains(stdout, `"cleanup_review_status": "not_eligible_public_provenance"`) ||
		strings.Contains(stdout, `"mandatory_retention_until"`) {
		t.Fatalf("goal evidence retention promotion provenance stdout drifted:\n%s", stdout)
	}
	if stderr != "" {
		t.Fatalf("goal evidence retention promotion provenance wrote stderr: %s", stderr)
	}

	futurePath := filepath.Join(t.TempDir(), "future-retained-evidence.json")
	futureArtifact := strings.ReplaceAll(terminalArtifact, `"retained_at": "2026-01-01T00:00:00Z"`, `"retained_at": "2026-06-20T00:00:00Z"`)
	if err := os.WriteFile(futurePath, []byte(futureArtifact), 0o644); err != nil {
		t.Fatalf("write future retained evidence fixture: %v", err)
	}
	code, stdout, stderr = runCLI("goal", "evidence", "retention", "--artifact", futurePath, "--now", now, "--json")
	if code != 1 {
		t.Fatalf("goal evidence retention future exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	if err := json.Unmarshal([]byte(stdout), &auditDocument); err != nil {
		t.Fatalf("goal evidence retention future --json emitted invalid JSON: %v\n%s", err, stdout)
	}
	if err := validateJSONSchemaValue(auditSchemaPath, auditDocument); err != nil {
		t.Fatalf("goal evidence retention future --json failed schema validation: %v\n%s", err, stdout)
	}
	if !strings.Contains(stdout, `"status": "failed"`) ||
		!strings.Contains(stdout, "is after now") {
		t.Fatalf("goal evidence retention future stdout drifted:\n%s", stdout)
	}
	if !strings.Contains(stderr, "retained evidence audit failed") {
		t.Fatalf("goal evidence retention future stderr drifted: %s", stderr)
	}
}
...
func TestGoalRunCLIDryRunsRetainedEvidenceCleanup(t *testing.T) {
	...
	for _, want := range []string{
		...
		"artifact=docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-command-rsi-manifest-retention-proof.json retention_status=active_retention cleanup_review_status=not_eligible_active_goal retention_class=loop_evidence",
		...
		"artifact=docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-foundry-rsi-next-improvement-task-retention-proof.json retention_status=active_retention cleanup_review_status=not_eligible_active_goal retention_class=loop_evidence",
		"artifact=docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/bounded-rsi-improvement-chain-retention-proof.json retention_status=active_retention cleanup_review_status=not_eligible_active_goal retention_class=loop_evidence",
		"artifact=docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-stack-rsi-chain-binding-readback-retention-proof.json retention_status=active_retention cleanup_review_status=not_eligible_active_goal retention_class=loop_evidence",
	} {
		if !strings.Contains(stdout, want) {
	...
	if summary.CleanupSchemaVersion != "ao.forge.goal-run-retained-evidence-cleanup.v0.1" ||
		...
		summary.Status != "passed" ||
		summary.ArtifactsScanned != 11 ||
		summary.EligibleArtifacts != 1 ||
		summary.ProtectedArtifacts != 10 ||
		summary.FailedArtifacts != 0 ||
		summary.PublicProvenanceExcluded != 2 ||
		summary.ActiveGoalExcluded != 8 ||
		len(summary.RetentionAudits) != 11 {
...
func TestGoalRunCLIReportsReadiness(t *testing.T) {
	...
	retainedPath := filepath.Join(root, "examples", "goals", "ao2-retained-evidence.goal-run.json")
	stalePath := filepath.Join(root, "examples", "goals", "invalid", "stale-evidence.goal-run.invalid.json")
	readinessSchemaPath := filepath.Join(root, "docs", "contracts", "goal-run-readiness-audit-v0.1.schema.json")
	now := "2026-06-19T18:00:00Z"

	code, stdout, stderr := runCLI("goal", "readiness", "--goal-run", retainedPath, "--to", "verification", "--now", now)
	if code != 0 {
		t.Fatalf("goal readiness exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	for _, want := range []string{
		"goal_readiness=passed",
		"goal_run=examples/goals/ao2-retained-evidence.goal-run.json",
		"goal_id=ao2-weekend-hardening",
		"current_phase=implementation",
		"requested_phase=verification",
		"check=goal_validate status=passed",
		"check=goal_inspect status=passed",
		"check=goal_transition status=passed",
	...
	if stderr != "" {
		t.Fatalf("goal readiness wrote stderr: %s", stderr)
	}

	code, stdout, stderr = runCLI("goal", "readiness", "--goal-run", retainedPath, "--to", "verification", "--now", now, "--json")
	if code != 0 {
		t.Fatalf("goal readiness --json exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	var summary struct {
		...
		RetentionAudits []struct {
			Status          string `json:"status"`
			RetentionStatus string `json:"retention_status"`
		} `json:"retention_audits"`
		Provenance struct {
			GoalRun struct {
				Path   string `json:"path"`
	...
	if err := json.Unmarshal([]byte(stdout), &summary); err != nil {
		t.Fatalf("goal readiness --json emitted invalid JSON: %v\n%s", err, stdout)
	}
	var readinessDocument any
	if err := json.Unmarshal([]byte(stdout), &readinessDocument); err != nil {
		t.Fatalf("goal readiness --json emitted invalid JSON document: %v\n%s", err, stdout)
	}
	if err := validateJSONSchemaValue(readinessSchemaPath, readinessDocument); err != nil {
		t.Fatalf("goal readiness --json failed schema validation: %v\n%s", err, stdout)
	}
	if summary.ReadinessSchemaVersion != "ao.forge.goal-run-readiness-audit.v0.1" ||
		summary.Status != "passed" ||
		summary.GoalID != "ao2-weekend-hardening" ||
		len(summary.Checks) != 6 ||
		len(summary.RetentionAudits) != 8 ||
		...
		summary.RetentionAudits[4].Status != "passed" ||
		summary.RetentionAudits[4].RetentionStatus != "active_retention" ||
		summary.RetentionAudits[5].Status != "passed" ||
		summary.RetentionAudits[5].RetentionStatus != "active_retention" ||
		summary.RetentionAudits[6].Status != "passed" ||
		summary.RetentionAudits[6].RetentionStatus != "active_retention" ||
		summary.RetentionAudits[7].Status != "passed" ||
		summary.RetentionAudits[7].RetentionStatus != "active_retention" {
		t.Fatalf("goal readiness JSON drifted: %+v", summary)
	}
	if stderr != "" {
		t.Fatalf("goal readiness --json wrote stderr: %s", stderr)
	}
	retainedBytes, err := os.ReadFile(retainedPath)
	if err != nil {
		t.Fatalf("read retained goal run: %v", err)
	}
	retainedSum := sha256.Sum256(retainedBytes)
	...
	if summary.Provenance.GoalRun.Path != "examples/goals/ao2-retained-evidence.goal-run.json" ||
		...
		len(summary.Provenance.Evidence) != 8 ||
		...
		summary.Provenance.Evidence[1].SHA256 != "2b8dd4f1d01069db8e679a56aba0eaa982dd8a02e446548af9325256f7d56702" ||
		summary.Provenance.Evidence[2].Path != "docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-command-rsi-manifest-retention-proof.json" ||
		summary.Provenance.Evidence[2].SHA256 != "e633c3925a25a40e903754de0ee441a809eba858d2c6341a95817640cf26f409" ||
		summary.Provenance.Evidence[3].Path != "docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-foundry-rsi-candidate-retention-proof.json" ||
		summary.Provenance.Evidence[3].SHA256 != "32946c7bfa3dbb768a0b868cd60ed07a3092306a321e98e6283673b797f6d580" ||
		summary.Provenance.Evidence[4].Path != "docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-foundry-rsi-improvement-gate-retention-proof.json" ||
		summary.Provenance.Evidence[4].SHA256 != "11efd2f2b59307cb188ec014a533b6f80d080fa2bfc470f257dceca2f4748ea1" ||
		summary.Provenance.Evidence[5].Path != "docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-foundry-rsi-next-improvement-task-retention-proof.json" ||
		summary.Provenance.Evidence[5].SHA256 != "e6ccfd723f5af775df2d0dafa53eb824e59f3f15df8e60ee1995fdf1fa59059e" ||
		summary.Provenance.Evidence[6].Path != "docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/bounded-rsi-improvement-chain-retention-proof.json" ||
		summary.Provenance.Evidence[6].SHA256 != "82cb13938f4ce05cc43e58b2e508de2a1f6e5004c6233f72045d215ea49c53d3" ||
		summary.Provenance.Evidence[7].Path != "docs/evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-stack-rsi-chain-binding-readback-retention-proof.json" ||
		summary.Provenance.Evidence[7].SHA256 != "a39a334b375b8ccd96a1173c24588854fcde9e7a9a5f48ab01668670e46527b2" {
		t.Fatalf("goal readiness provenance drifted: %+v", summary.Provenance)
	}

	code, stdout, stderr = runCLI("goal", "readiness", "--goal-run", retainedPath, "--to", "complete", "--now", now, "--json")
	if code != 1 {
		t.Fatalf("goal readiness denied transition exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	if err := json.Unmarshal([]byte(stdout), &readinessDocument); err != nil {
		t.Fatalf("goal readiness denied transition --json emitted invalid JSON: %v\n%s", err, stdout)
	}
	if err := validateJSONSchemaValue(readinessSchemaPath, readinessDocument); err != nil {
		t.Fatalf("goal readiness denied transition --json failed schema validation: %v\n%s", err, stdout)
	}
	if !strings.Contains(stdout, `"status": "failed"`) ||
		!strings.Contains(stdout, "goal_transition") {
		t.Fatalf("goal readiness denied transition stdout drifted:\n%s", stdout)
	}
	if !strings.Contains(stderr, "readiness audit failed") {
		t.Fatalf("goal readiness denied transition stderr drifted: %s", stderr)
	}

	code, stdout, stderr = runCLI("goal", "readiness", "--goal-run", stalePath, "--now", now, "--json")
	if code != 1 {
		t.Fatalf("goal readiness stale evidence exit code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	if err := json.Unmarshal([]byte(stdout), &readinessDocument); err != nil {
		t.Fatalf("goal readiness stale evidence --json emitted invalid JSON: %v\n%s", err, stdout)
	}
	if err := validateJSONSchemaValue(readinessSchemaPath, readinessDocument); err != nil {
		t.Fatalf("goal readiness stale evidence --json failed schema validation: %v\n%s", err, stdout)
	}
	if !strings.Contains(stdout, "evidence_verify") ||
		!strings.Contains(stdout, "sha256 mismatch") {
		t.Fatalf("goal readiness stale evidence stdout drifted:\n%s", stdout)
	}
	if !strings.Contains(stderr, "readiness audit failed") {
		t.Fatalf("goal readiness stale evidence stderr drifted: %s", stderr)
	}
}
...
func TestFactoryBriefAndPlanSchemasAreLinkedAndStrict(t *testing.T) {
	...
	goalRunVerificationSchema := readText("docs", "contracts", "goal-run-verification-v0.1.schema.json")
	...
	goalRunVerificationExample := readText("examples", "goals", "ao2-weekend-hardening.goal-run-verification.json")
	...
	rsiManifestRetainedEvidenceArtifact := readText("docs", "evidence", "goals", "ao2-weekend-hardening", "20260619T180000Z-verification", "ao-command-rsi-manifest-retention-proof.json")
	...
	releaseVerifyAuditSchema := readText("docs", "contracts", "release-verify-audit-v0.1.schema.json")
	...
	for _, check := range []struct {
		...
		{name: "docs index goal run verification schema link", doc: docsIndex, want: "[GoalRun Verification v0.1 Schema](contracts/goal-run-verification-v0.1.schema.json)"},
		...
		{name: "docs index goal run verification example link", doc: docsIndex, want: "[Example GoalRun Verification](../examples/goals/ao2-weekend-hardening.goal-run-verification.json)"},
		...
		{name: "docs index command RSI manifest proof link", doc: docsIndex, want: "[AO Command RSI Manifest Retention Proof](evidence/goals/ao2-weekend-hardening/20260619T180000Z-verification/ao-command-rsi-manifest-retention-proof.json)"},
		...
		{name: "README goal verification validate command", doc: readme, want: "forge goal verification validate --verification examples/goals/ao2-weekend-hardening.goal-run-verification.json"},
		{name: "README goal verification schema validate command", doc: readme, want: "forge contract validate --schema docs/contracts/goal-run-verification-v0.1.schema.json --document examples/goals/ao2-weekend-hardening.goal-run-verification.json"},
		...
		{name: "goal run verification schema id", doc: goalRunVerificationSchema, want: `"ao.forge.goal-run-verification.v0.1"`},
		{name: "goal run verification strict root", doc: goalRunVerificationSchema, want: `"additionalProperties": false`},
		{name: "goal run verification phases", doc: goalRunVerificationSchema, want: `"security_scan"`},
		{name: "goal run verification security review", doc: goalRunVerificationSchema, want: `"security_review"`},
		{name: "goal run verification non-mutating", doc: goalRunVerificationSchema, want: `"mutates_live_state"`},
		{name: "goal run verification example schema", doc: goalRunVerificationExample, want: `"schema_version": "ao.forge.goal-run-verification.v0.1"`},
		{name: "goal run verification example public readiness", doc: goalRunVerificationExample, want: `"phase": "public_readiness"`},
		...
		{name: "goal run evidence lint schema id", doc: goalRunEvidenceLintSchema, want: `"ao.forge.goal-run-evidence-lint.v0.1"`},
		{name: "goal run evidence lint strict root", doc: goalRunEvidenceLintSchema, want: `"additionalProperties": false`},
		...
		{name: "goal run retained evidence metadata", doc: goalRunRetainedEvidenceSchema, want: `"retention_metadata"`},
		{name: "goal run retained evidence retained at", doc: goalRunRetainedEvidenceSchema, want: `"retained_at"`},
		{name: "goal run retained evidence retention class", doc: goalRunRetainedEvidenceSchema, want: `"loop_evidence"`},
		{name: "goal run retained evidence release provenance class", doc: goalRunRetainedEvidenceSchema, want: `"release_provenance"`},
		{name: "goal run retained evidence promotion provenance class", doc: goalRunRetainedEvidenceSchema, want: `"promotion_provenance"`},
		{name: "goal run retained evidence active retention", doc: goalRunRetainedEvidenceSchema, want: `"retain_while_goal_active"`},
		{name: "goal run retained evidence deletion review", doc: goalRunRetainedEvidenceSchema, want: `"deletion_requires_review"`},
		{name: "goal run retained evidence captured outputs", doc: goalRunRetainedEvidenceSchema, want: `"captured_outputs"`},
		{name: "goal run retained evidence RSI mode property", doc: goalRunRetainedEvidenceSchema, want: `"rsi_mode"`},
		{name: "goal run retained evidence claim levels property", doc: goalRunRetainedEvidenceSchema, want: `"claim_levels"`},
		{name: "goal run retained evidence bounded RSI claim", doc: goalRunRetainedEvidenceSchema, want: `"bounded_governed_rsi"`},
		{name: "goal run retained evidence full RSI claim", doc: goalRunRetainedEvidenceSchema, want: `"full_autonomous_self_mutating_rsi"`},
		{name: "goal run retained evidence audit schema id", doc: goalRunRetainedEvidenceAuditSchema, want: `"ao.forge.goal-run-retained-evidence-audit.v0.1"`},
		{name: "goal run retained evidence audit strict root", doc: goalRunRetainedEvidenceAuditSchema, want: `"additionalProperties": false`},
		{name: "goal run retained evidence audit active status", doc: goalRunRetainedEvidenceAuditSchema, want: `"active_retention"`},
		...
		{name: "goal run docs tampered readiness audit fixture", doc: goalRunDocs, want: "examples/goals/invalid/tampered-readiness-audit.goal-run-readiness-audit.invalid.json"},
		...
		{name: "goal run docs no scratch persisted evidence", doc: goalRunDocs, want: "must not be preserved in the durable GoalRun or update audit"},
		...
		{name: "goal run docs public provenance not cleanup eligible", doc: goalRunDocs, want: "not_eligible_public_provenance"},
		...
		{name: "goal run docs verification command", doc: goalRunDocs, want: "forge goal verification validate"},
		...
		{name: "AO2 Pulse docs evidence verify command", doc: ao2PulseGoalRunLoopDocs, want: "forge goal evidence verify --goal-run tmp/ao2-weekend-hardening.goal-run.json"},
		{name: "AO2 Pulse docs evidence lint command", doc: ao2PulseGoalRunLoopDocs, want: "forge goal evidence lint --goal-run tmp/ao2-weekend-hardening.goal-run.json"},
		...
		{name: "AO2 Pulse docs retain handoff step", doc: ao2PulseGoalRunLoopDocs, want: "Retain the handoff evidence"},
		{name: "AO2 Pulse docs retained evidence layout", doc: ao2PulseGoalRunLoopDocs, want: "docs/evidence/goals/<goal_id>/<YYYYMMDDTHHMMSSZ>-<phase>/"},
		{name: "AO2 Pulse docs no tmp evidence", doc: ao2PulseGoalRunLoopDocs, want: "Do not record"},
		{name: "AO2 Pulse docs public evidence indefinitely", doc: ao2PulseGoalRunLoopDocs, want: "public release or promotion evidence is retained indefinitely"},
		{name: "AO2 Pulse docs stale stop condition", doc: ao2PulseGoalRunLoopDocs, want: "evidence is duplicated, stale, or no longer proves the next task"},
		...
		{name: "retained evidence GoalRun references RSI manifest", doc: retainedEvidenceGoalRunExample, want: "ao-command-rsi-manifest-retention-proof.json"},
		...
		{name: "retained evidence artifact retained at", doc: retainedEvidenceArtifact, want: `"retained_at": "2026-06-19T14:30:00Z"`},
		{name: "retained evidence artifact retention class", doc: retainedEvidenceArtifact, want: `"retention_class": "loop_evidence"`},
		...
		{name: "RSI health retained evidence artifact schema version", doc: rsiHealthRetainedEvidenceArtifact, want: `"schema_version": "ao.forge.goal-run-retained-evidence.v0.1"`},
		...
		{name: "RSI health retained evidence artifact claim levels", doc: rsiHealthRetainedEvidenceArtifact, want: `"claim_levels"`},
		{name: "RSI health retained evidence artifact bounded claim", doc: rsiHealthRetainedEvidenceArtifact, want: `"claim": "bounded_governed_rsi"`},
		{name: "RSI health retained evidence artifact full claim", doc: rsiHealthRetainedEvidenceArtifact, want: `"claim": "full_autonomous_self_mutating_rsi"`},
		{name: "RSI manifest retained evidence artifact schema version", doc: rsiManifestRetainedEvidenceArtifact, want: `"schema_version": "ao.forge.goal-run-retained-evidence.v0.1"`},
		{name: "RSI manifest retained evidence artifact output", doc: rsiManifestRetainedEvidenceArtifact, want: `"command": "ao-command rsi manifest --manifest ../ao-architecture/overview/rsi-claim-evidence-manifest.json"`},
		{name: "RSI manifest retained evidence artifact command schema", doc: rsiManifestRetainedEvidenceArtifact, want: `"schema_version": "ao.command.rsi-manifest.v0.1"`},
		{name: "RSI manifest retained evidence artifact rollback marker", doc: rsiManifestRetainedEvidenceArtifact, want: `"rollback_rehearsal.status=passed"`},
		{name: "RSI manifest retained evidence artifact command PR", doc: rsiManifestRetainedEvidenceArtifact, want: `"AO Command PR #31"`},
		{name: "RSI manifest retained evidence artifact mutation boundary", doc: rsiManifestRetainedEvidenceArtifact, want: `"mutates_repositories": false`},
		...
		{name: "retained readiness audit goal run sha", doc: retainedReadinessAudit, want: `"sha256": "2331e7b9c674cb6d747b314606004c97b4cf22991472238ee522c9791dc003a3"`},
		{name: "retained readiness audit command RSI manifest proof", doc: retainedReadinessAudit, want: "ao-command-rsi-manifest-retention-proof.json"},
		...
		{name: "retained readiness audit Foundry RSI next task proof", doc: retainedReadinessAudit, want: "ao-foundry-rsi-next-improvement-task-retention-proof.json"},
		{name: "retained readiness audit bounded RSI chain proof", doc: retainedReadinessAudit, want: "bounded-rsi-improvement-chain-retention-proof.json"},
		{name: "retained readiness audit AO stack RSI chain-binding readback proof", doc: retainedReadinessAudit, want: "ao-stack-rsi-chain-binding-readback-retention-proof.json"},
		...
		{name: "release attestation plan schema id", doc: releaseAttestationPlanSchema, want: `"ao.forge.release-attestation-plan.v0.1"`},
		{name: "release attestation plan strict root", doc: releaseAttestationPlanSchema, want: `"additionalProperties": false`},
		{name: "release attestation plan subjects", doc: releaseAttestationPlanSchema, want: `"subjects"`},
		...
		{name: "release verify audit schema id", doc: releaseVerifyAuditSchema, want: `"ao.forge.release-verify-audit.v0.1"`},
		{name: "release verify audit strict root", doc: releaseVerifyAuditSchema, want: `"additionalProperties": false`},
		{name: "release verify audit controls", doc: releaseVerifyAuditSchema, want: `"require_evidence_bundle"`},
		{name: "release verify audit checks", doc: releaseVerifyAuditSchema, want: `"release_evidence_bundle_attestation_verified"`},
	...
	for _, doc := range []struct {
		...
		{name: "release verify audit schema", text: releaseVerifyAuditSchema},
...
func TestReleaseAuditContractsValidateWorkflowEvidence(t *testing.T) {
	root := repoRoot(t)
	verifySchema := filepath.Join(root, "docs", "contracts", "release-verify-audit-v0.1.schema.json")
	...
	verifyAudit := []byte(`{
  "schema_version": "ao.forge.release-verify-audit.v0.1",
  "status": "passed",
  "tag": "v0.1.0",
  ...
  "checks": {
    ...
    "release_tag_identity_verified": true,
    "checksums_verified": true,
    "release_preview_inspect_verified": true,
    "archive_attestations_verified": true,
    "release_evidence_bundle_verified": true,
    "release_evidence_bundle_attestation_verified": true,
    "binary_smoke_test_passed_or_skipped": true
  },
  "verified_assets": [
	...
	verifyDoc := filepath.Join(tmp, "release-verify-audit.json")
	if err := os.WriteFile(verifyDoc, verifyAudit, 0o600); err != nil {
		t.Fatalf("write verify audit sample: %v", err)
	}
  ...
  "criteria": {
    ...
    "release_verify_audit_run_matches": true,
    "release_verify_audit_tag_matches": true,
    "release_verify_audit_release_matches": true,
    "release_verify_default_controls": true,
    "release_install_verify_succeeded": true,
    "install_verify_audit_run_matches": true,
    "install_verify_audit_tag_matches": true,
    "install_verify_audit_release_matches": true,
    "rollback_audit_succeeded": true,
    "rollback_audit_only": true,
    "rollback_release_unchanged": true
  },
  "failed_criteria": [],
  "operator_assertions": {
  ...
  "operator_rules": [
	...
	for _, tc := range []struct {
		...
		{name: "release verify audit", schema: verifySchema, document: verifyDoc},
...
func TestReleaseAttestationPlanContractDocumentsSignedEvidence(t *testing.T) {
	root := repoRoot(t)
	readText := func(path ...string) string {
		t.Helper()
		bytes, err := os.ReadFile(filepath.Join(append([]string{root}, path...)...))
	...
	schemaPath := filepath.Join(root, "docs", "contracts", "release-attestation-plan-v0.1.schema.json")
	examplePath := filepath.Join(root, "examples", "release-preview", "release-attestation-plan.v0.1.example.json")
	...
	code, stdout, stderr := runCLI("contract", "validate", "--schema", schemaPath, "--document", examplePath)
	if code != 0 {
		t.Fatalf("release attestation plan example failed validation with code %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	for _, want := range []string{
		"contract_validation=passed",
		"schema=" + displayPath(schemaPath),
...
func TestLiveMutationDryRunPlanContractValidatesAuthorityIsolationAndRollback(t *testing.T) {
	...
	code, stdout, stderr = runCLI("contract", "validate", "--schema", schemaPath, "--document", docsMultiValidPath)
	if code != 0 {
		t.Fatalf("docs multi live mutation dry-run plan fixture failed validation with code %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	...
	code, stdout, stderr = runCLI("contract", "validate", "--schema", schemaPath, "--document", codeClassInvalidPath)
	if code != 1 {
		t.Fatalf("code-class dry-run plan should fail validation, code = %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
}
...
func TestLiveDocsExecutionGuardWritesReadyResult(t *testing.T) {
	...
	if !ok {
		t.Fatalf("guard missing mutation_class_policy: %#v", guard)
	}
	if policy["authority_boundary"] != "docs_only_classes_only" {
		t.Fatalf("unexpected mutation class policy: %#v", policy)
	}
	if !stringSliceContainsAny(policy["permitted_classes"], "docs_only_multi_file") ||
		!stringSliceContainsAny(policy["denied_classes"], "low_risk_code") {
		t.Fatalf("mutation class policy does not expose docs-only permit/code deny boundary: %#v", policy)
	}
}

func TestLiveDocsExecutionGuardPermitsDocsOnlyMultiFileClass(t *testing.T) {
	root := repoRoot(t)
	out := filepath.Join(t.TempDir(), "guard.json")
	code, stdout, stderr := runCLI(
		"live-docs", "guard",
		"--plan", filepath.Join(root, "examples", "live-mutation", "docs-multi-dry-run-plan.json"),
		"--approval-gate", filepath.Join(root, "examples", "live-docs-guard", "foundry-class-gate.docs-multi.ready.json"),
		"--ticket", filepath.Join(root, "examples", "live-docs-guard", "covenant-class-ticket.docs-multi.approved.json"),
		"--sentinel", filepath.Join(root, "examples", "live-docs-guard", "sentinel-class.no-hold.docs-multi.json"),
		"--command-readback", filepath.Join(root, "examples", "live-docs-guard", "command-authority-ladder.docs-multi.ready.json"),
		"--out", out,
	)
	if code != 0 {
		t.Fatalf("live-docs guard docs multi exit=%d\nstdout=%s\nstderr=%s", code, stdout, stderr)
	}
	if !strings.Contains(stdout, "safe_to_execute=true") {
		t.Fatalf("stdout missing safe_to_execute=true:\n%s", stdout)
	}
	var guard map[string]any
	readJSONFixture(t, out, &guard)
	if guard["status"] != "ready" || guard["safe_to_execute"] != true {
		t.Fatalf("docs multi guard did not pass: %#v", guard)
	}
	scope, ok := guard["approved_scope"].(map[string]any)
	if !ok || scope["mutation_class"] != "docs_only_multi_file" {
		t.Fatalf("docs multi approved scope missing mutation class: %#v", guard["approved_scope"])
	}
	policy, ok := guard["mutation_class_policy"].(map[string]any)
	if !ok || policy["current_class"] != "docs_only_multi_file" {
		t.Fatalf("docs multi policy missing current class: %#v", guard["mutation_class_policy"])
	}
}

func TestLiveDocsExecutionGuardPermitsTestOnlyClass(t *testing.T) {
	root := repoRoot(t)
	out := filepath.Join(t.TempDir(), "guard.json")
	code, stdout, stderr := runCLI(
		"live-docs", "guard",
		"--plan", filepath.Join(root, "examples", "live-mutation", "test-only-dry-run-plan.json"),
		"--approval-gate", filepath.Join(root, "examples", "live-docs-guard", "foundry-class-gate.test-only.ready.json"),
		"--ticket", filepath.Join(root, "examples", "live-docs-guard", "covenant-class-ticket.test-only.approved.json"),
		"--sentinel", filepath.Join(root, "examples", "live-docs-guard", "sentinel-class.no-hold.test-only.json"),
		"--command-readback", filepath.Join(root, "examples", "live-docs-guard", "command-authority-ladder.test-only.ready.json"),
		"--out", out,
	)
	if code != 0 {
		t.Fatalf("live-docs guard test-only exit=%d\nstdout=%s\nstderr=%s", code, stdout, stderr)
	}
	if !strings.Contains(stdout, "safe_to_execute=true") {
		t.Fatalf("stdout missing safe_to_execute=true:\n%s", stdout)
	}
	...
	readJSONFixture(t, out, &guard)
	if guard["status"] != "ready" || guard["safe_to_execute"] != true {
		t.Fatalf("test-only guard did not pass: %#v", guard)
	}
	if guard["allowed_next_action"] != "prepare_isolated_test_only_branch" {
		t.Fatalf("test-only guard exposed unexpected next action: %#v", guard["allowed_next_action"])
	}
	scope, ok := guard["approved_scope"].(map[string]any)
	if !ok || scope["mutation_class"] != "test_only" || scope["allowed_path_class"] != "test_only" {
		t.Fatalf("test-only approved scope missing class or path class: %#v", guard["approved_scope"])
	}
	paths, ok := scope["allowed_paths"].([]any)
	if !ok || len(paths) != 1 || paths[0] != "internal/atlas/atlas_test.go" {
		t.Fatalf("test-only approved scope widened paths: %#v", scope["allowed_paths"])
	}
	policy, ok := guard["mutation_class_policy"].(map[string]any)
	if !ok || policy["current_class"] != "test_only" || policy["authority_boundary"] != "test_only_class_only" {
		t.Fatalf("test-only policy missing boundary: %#v", guard["mutation_class_policy"])
	}
	if !stringSliceContainsAny(policy["denied_classes"], "low_risk_code") {
		t.Fatalf("test-only policy must still deny code classes: %#v", policy)
	}
}
...
func TestLiveDocsExecutionGuardBlocksLowRiskCodeWidenedPath(t *testing.T) {
	...
	code, stdout, stderr := runCLI(
		"live-docs", "guard",
		"--plan", filepath.Join(root, "examples", "live-mutation", "invalid", "low-risk-code.dry-run-plan.invalid.json"),
		"--approval-gate", filepath.Join(root, "examples", "live-docs-guard", "foundry-approval-gate.ready.json"),
		"--ticket", filepath.Join(root, "examples", "live-docs-guard", "covenant-ticket.approved.json"),
		"--sentinel", filepath.Join(root, "examples", "live-docs-guard", "sentinel.no-hold.json"),
		"--command-readback", filepath.Join(root, "examples", "live-docs-guard", "command-readback.ready.json"),
...
func TestLiveDocsExecutionGuardContractFixtures(t *testing.T) {
	...
	code, stdout, stderr = runCLI("contract", "validate", "--schema", schemaPath, "--document", docsMultiValidPath)
	if code != 0 {
		t.Fatalf("valid docs multi live docs guard fixture failed code=%d\nstdout=%s\nstderr=%s", code, stdout, stderr)
	}
	code, stdout, stderr = runCLI("contract", "validate", "--schema", schemaPath, "--document", testOnlyValidPath)
	if code != 0 {
...
func TestBranchProtectionRunbookDocumentsRequiredChecks(t *testing.T) {
	...
	for _, check := range []struct {
		...
		{name: "goal fixture protection explanation", doc: runbook, want: "evidence verifier JSON"},
		{name: "production readiness protection explanation", doc: runbook, want: "machine-readable"},
		{name: "production readiness artifact explanation", doc: runbook, want: "machine-readable artifacts before merge"},
		{name: "production readiness cleanup command explanation", doc: runbook, want: "forge goal evidence cleanup --dry-run --json"},
		{name: "production readiness cleanup schema explanation", doc: runbook, want: "goal-run-retained-evidence-cleanup-v0.1.schema.json"},
		...
		{name: "goal fixture verifier readiness audit rejected count", doc: goalFixtureVerifier, want: "goal_run_readiness_audit_invalid_fixtures_rejected"},
...
func TestReleasePreviewWorkflowPublishesDryRunAuditArtifacts(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Validate release preview audit schema",
		"Validate release preview inspect schema",
		"contract validate --schema docs/contracts/release-preview-audit-v0.1.schema.json",
		"contract validate --schema docs/contracts/release-preview-inspect-v0.1.schema.json",
...
func TestReleaseAttestationWorkflowProducesSignedEvidenceWithoutPublishing(t *testing.T) {
	...
	for _, check := range []struct {
		...
		{name: "portable checksum cwd", doc: workflow, want: "cd dist"},
		{name: "portable linux checksum path", doc: workflow, want: "--artifact ao-forge_Linux_x86_64.tar.gz"},
		{name: "portable darwin checksum path", doc: workflow, want: "--artifact ao-forge_Darwin_arm64.tar.gz"},
		{name: "portable windows checksum path", doc: workflow, want: "--artifact ao-forge_Windows_x86_64.zip"},
...
func TestReleasePublishWorkflowCreatesDraftReleaseOnlyAfterEvidenceGates(t *testing.T) {
	...
	for _, check := range []struct {
		...
		{name: "tag default", doc: workflow, want: "default: \"v0.1.4\""},
		{name: "exact source input", doc: workflow, want: "source_commit:"},
		{name: "rehearsal run input", doc: workflow, want: "release_rehearsal_run_id:"},
		{name: "plan digest input", doc: workflow, want: "expected_plan_digest:"},
		{name: "source-owned release notes", doc: workflow, want: "RELEASE_NOTES_PATH=docs/release/V${VERSION}-RELEASE-NOTES.md"},
...
func TestReleaseVerifyWorkflowChecksPublishedReleaseEvidence(t *testing.T) {
	...
	for _, check := range []struct {
		...
		{name: "signed tag default", doc: workflow, want: "default: \"true\""},
		{name: "read contents", doc: workflow, want: "contents: read"},
		...
		{name: "signed tag resolver", doc: workflow, want: "AO_FORGE_REQUIRE_SIGNED_TAG"},
		...
		{name: "write verify audit", doc: workflow, want: "Write release verify audit"},
		{name: "verify audit schema", doc: workflow, want: "ao.forge.release-verify-audit.v0.1"},
		{name: "verify audit contract", doc: workflow, want: `contract validate --schema docs/contracts/release-verify-audit-v0.1.schema.json --document "${AO_FORGE_RELEASE_VERIFY_DIR}/release-verify-audit.json"`},
...
func TestReleaseRollbackWorkflowGuardsReleaseYankActions(t *testing.T) {
	...
	for _, check := range []struct {
		...
		{name: "read-only audit job", doc: workflow, want: "rollback-audit:"},
		{name: "read-only audit name", doc: workflow, want: "name: Read-only rollback audit"},
		{name: "mutating rollback job", doc: workflow, want: "rollback-mutation:"},
		{name: "mutating rollback conditional", doc: workflow, want: "if: ${{ github.event.inputs.rollback_action != 'audit-only' }}"},
		{name: "environment approval", doc: workflow, want: "environment: production-release"},
		{name: "top-level contents read", doc: workflow, want: "contents: read"},
		{name: "mutation contents write", doc: workflow, want: "contents: write"},
...
func TestProductionPromotionWorkflowDefinesStableCriteria(t *testing.T) {
	...
	for _, check := range []struct {
		...
		{name: "solo operator soak default", doc: workflow, want: "default: \"0\""},
		{name: "default verify assertion input", doc: workflow, want: "confirm_default_release_verify:"},
		{name: "blocker assertion input", doc: workflow, want: "confirm_no_known_blockers:"},
		...
		{name: "default verify assertion gate", doc: workflow, want: "confirm_default_release_verify must be exactly true"},
		...
		{name: "download release verify evidence", doc: workflow, want: "Download release verification evidence"},
		{name: "release verify artifact", doc: workflow, want: "release-verify-audit"},
		{name: "release verify contract validation", doc: workflow, want: `contract validate --schema docs/contracts/release-verify-audit-v0.1.schema.json --document "${AO_FORGE_PROMOTION_OUT}/verify/release-verify-audit.json"`},
		{name: "install verify run metadata", doc: workflow, want: "release-install-verify-run.json"},
		{name: "download install verify evidence", doc: workflow, want: "Download install verification evidence"},
		{name: "install verify artifact", doc: workflow, want: "release-install-verify-audit"},
		{name: "install verify contract validation", doc: workflow, want: `contract validate --schema docs/contracts/release-install-verify-audit-v0.1.schema.json --document "${AO_FORGE_PROMOTION_OUT}/install/release-install-verify-audit.json"`},
		...
		{name: "verify audit loaded", doc: workflow, want: `verify_audit = json.loads((out / "verify" / "release-verify-audit.json").read_text())`},
		{name: "verify audit release loaded", doc: workflow, want: `verify_release = verify_audit.get("release", {})`},
		{name: "verify audit run binding", doc: workflow, want: `"release_verify_audit_run_matches": verify_audit.get("workflow", {}).get("run_id") == str(verify_run.get("databaseId"))`},
		{name: "verify audit tag binding", doc: workflow, want: `"release_verify_audit_tag_matches": verify_audit.get("tag") == tag`},
		{name: "verify audit target binding", doc: workflow, want: `"release_verify_audit_release_matches": verify_release.get("target_commitish") == release.get("targetCommitish")`},
		{name: "verify default controls", doc: workflow, want: `"release_verify_default_controls": verify_audit.get("controls", {}).get("require_evidence_bundle") is True and verify_audit.get("controls", {}).get("require_signed_tag") is True`},
		{name: "install verify workflow required", doc: workflow, want: "Release Install Verify"},
		{name: "install audit loaded", doc: workflow, want: `install_verify_audit = json.loads((out / "install" / "release-install-verify-audit.json").read_text())`},
		{name: "install audit run binding", doc: workflow, want: `"install_verify_audit_run_matches": install_verify_audit.get("workflow", {}).get("run_id") == str(install_verify_run.get("databaseId"))`},
		{name: "install audit tag binding", doc: workflow, want: `"install_verify_audit_tag_matches": install_verify_audit.get("tag") == tag`},
		{name: "install audit release loaded", doc: workflow, want: `install_verify_release = install_verify_audit.get("release", {})`},
		{name: "install audit target binding", doc: workflow, want: `"install_verify_audit_release_matches": install_verify_release.get("target_commitish") == release.get("targetCommitish")`},
		...
		{name: "blocked audit does not fail before upload", doc: workflow, want: "Production promotion criteria failed; audit status is blocked."},
		...
		{name: "enforce after upload", doc: workflow, want: "Enforce production promotion result"},
		{name: "enforce reads uploaded audit", doc: workflow, want: `audit = json.loads((out / "production-promotion-audit.json").read_text())`},
		...
		{name: "runbook blocked audit upload", doc: runbook, want: "contract-valid audit artifact before the workflow fails"},
		{name: "runbook solo default soak", doc: runbook, want: "The default soak window is 0 hours for solo-operator controlled releases."},
		{name: "runbook public user soak", doc: runbook, want: "external users depend on the release"},
		{name: "runbook verify audit binding", doc: runbook, want: "Verify audit artifact must validate against its contract"},
		{name: "runbook verify default controls", doc: runbook, want: "signed-tag and evidence-bundle controls"},
		...
		{name: "runbook install audit binding", doc: runbook, want: "verification audit artifact must validate against its contract"},
		{name: "runbook install audit commit binding", doc: runbook, want: "promoted tag and release commit"},
