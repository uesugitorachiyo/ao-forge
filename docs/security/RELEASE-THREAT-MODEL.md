...
| Artifact tampering | An artifact changes between build and release review. | `forge artifact checksums` emits a stable SHA-256 manifest, and `forge release-preview` records artifact size, checksum, status, and provenance. | `checksums.txt` and release preview artifact details. | A future signed checksum attestation can bind checksums to an operator key. |
